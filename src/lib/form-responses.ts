...
export type QuestionSummary =
  | {
      ...
      title: string;
      type: 'choice';
      /** single = pick-one (pie chart, like Google Forms); multi = checkboxes (bar chart). */
...
function blockSummaryKind(
  block: EditorBlock | undefined
): { kind: 'choice'; variant: 'single' | 'multi' } | { kind: 'scale' | 'text' | 'other' } {
  if (!block) return { kind: 'other' };
  if (block.kind === 'multiple_choice' || block.kind === 'dropdown') {
    return { kind: 'choice', variant: 'single' };
  }
  if (block.kind === 'checkboxes') return { kind: 'choice', variant: 'multi' };
  if (block.kind === 'linear_scale') return { kind: 'scale' };
  if (block.kind === 'short_text' || block.kind === 'paragraph') return { kind: 'text' };
  return { kind: 'other' };
}
